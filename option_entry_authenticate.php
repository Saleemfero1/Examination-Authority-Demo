...
if ($captacha!=$captacha_check) {
	echo"<script>confirm('incorrect captacha please retry with proper captacha');window.location.href='option_entry_login.php'</script>";
}
	...
	$sql= "SELECT application_id,password,ranking FROM $table where application_id='$application_id'";
	...
	else
	{
			...
			if ($row['application_id']) 
			{
				if($password_fetched=='NULL')
					echo"<script>confirm('please reset u your password and login');window.location.href='option_entry_login.php'</script>";

				if($password_fetched==NULL)
					echo"<script>confirm('please reset u your password and login');window.location.href='option_entry_login.php'</script>";

				if ($password_str!=$password_fetched) {
					echo"<script>confirm('Incorrect Password');window.location.href='option_entry_login.php'</script>";
				}
					if($password_str==$password_fetched){
						$_SESSION["ranking"]=$ranking;
						$_SESSION["admission_type"]=$selected_admission;
						$_SESSION["application_id"]=$row['application_id'];
							echo"<script>window.location.href='http://localhost/EAD%20project/students_application/option_entry.php'</script>";
				}
			}
			else
			{
